...
const WEB_CHANNEL_HINT =
  ...
  'For diagrams: when a flowchart/sequence/state/ER/architecture sketch helps, emit a valid ```mermaid fence in your answer (WebUI renders it inline). Prefer one focused diagram. Always double-quote node labels that contain parentheses or math (B{"g(x) ≥ 0?"}, A["f(x)"]) — bare B{g(x)} / A[g(x)] fails parse.\n' +
  'For maps: call show_map when a place map helps; paste the WEB ONLY ```map fence once; never invent map fences.\n' +
  'For information cards: call show_card when structured facts benefit from a designed card/deck; include the summary lines; never invent card fences (WebUI host injects the fence if omitted).\n' +
